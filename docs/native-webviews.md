...
- **Closing saves.** Any other unmount cannot reach the page any more, so the session saves whatever the model holds. A "Close without saving" confirmation suspends autosave first, and a suspended model discards instead of saving.
